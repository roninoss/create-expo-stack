...
export async function printOutput(
  ...
  if (!options.noGit && !flags.noGit && process.env.NODE_ENV !== 'test') {
    ...
    // get rn.new version
    const rnNewVersion: string = require('../../package.json').version || '2.0.0';

    await runSystemCommand({
      toolbox,
      command: `cd ${projectName} && git init --quiet && git add . && git commit -m "Initial commit" -m "Generated by rn-new ${rnNewVersion}" --quiet`,
